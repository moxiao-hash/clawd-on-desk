...
const updateRegistry = {
  ...
  musicDanceEnabled: requireBoolean("musicDanceEnabled"),
  musicAppNames(value) {
    if (!Array.isArray(value)) {
      return { status: "error", message: "musicAppNames must be an array" };
    }
    for (let i = 0; i < value.length; i++) {
      if (typeof value[i] !== "string" || value[i].length === 0) {
        return { status: "error", message: `musicAppNames[${i}] must be a non-empty string` };
      }
    }
    return { status: "ok" };
  },
