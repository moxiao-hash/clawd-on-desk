...
(function initSettingsTabGeneral(root) {
  ...
  function render(parent) {
    ...
    parent.appendChild(buildMusicDanceSection());
  }
  ...
  function buildMusicDanceSection() {
    const section = helpers.buildSection(t("sectionMusicDance"), [
      helpers.buildSwitchRow({
        key: "musicDanceEnabled",
        labelKey: "rowMusicDanceEnabled",
        descKey: "rowMusicDanceEnabledDesc",
      }),
    ]);

    // Build the app names tag editor row
    const apps = state.snapshot && Array.isArray(state.snapshot.musicAppNames)
      ? [...state.snapshot.musicAppNames]
      : [];
    const enabled = !!(state.snapshot && state.snapshot.musicDanceEnabled);
    const tagRow = buildMusicAppTagRow(apps, enabled);
    section.appendChild(tagRow);
    // Tag row visibility toggles with the switch
    const updateTagVisibility = () => {
      const raw = !!(state.snapshot && state.snapshot.musicDanceEnabled);
      tagRow.style.display = raw ? "" : "none";
    };
    // Use MutationObserver to detect switch visual changes
    const switchEl = tagRow.previousElementSibling;
    if (switchEl) {
      const sw = switchEl.querySelector(".switch");
      if (sw) {
        new MutationObserver(updateTagVisibility).observe(sw, { attributes: true, attributeFilter: ["class"] });
      }
    }
    updateTagVisibility();
    return section;
  ...
  function buildMusicAppTagRow(apps, enabled) {
    const row = document.createElement("div");
    row.className = "row music-app-tag-row";
    if (!enabled) row.style.display = "none";

    const text = document.createElement("div");
    text.className = "row-text";
    const label = document.createElement("span");
    label.className = "row-label";
    label.textContent = t("rowMusicAppNames");
    const desc = document.createElement("span");
    desc.className = "row-desc";
    desc.textContent = t("rowMusicAppNamesDesc");
    text.appendChild(label);
    text.appendChild(desc);

    const control = document.createElement("div");
    control.className = "row-control music-app-tags";

    function renderTags() {
      // Clear existing tags
      while (control.children.length > 1) {
        control.removeChild(control.lastChild);
      }
      // Always keep the add input as last child (created after this call)

      apps.forEach((name, idx) => {
        const chip = document.createElement("span");
        ...
        control.insertBefore(chip, addGroup);
      });
    }

    function commitApps() {
      renderTags();
      window.settingsAPI.update("musicAppNames", [...apps]).then((result) => {
        if (result && result.status === "error") {
          // Restore from snapshot on error
          apps.length = 0;
          const fromStore = state.snapshot && Array.isArray(state.snapshot.musicAppNames)
            ? state.snapshot.musicAppNames : [];
          apps.push(...fromStore);
          renderTags();
        }
    ...
    input.placeholder = t("rowMusicAppNamesPlaceholder") || "App name";
    input.maxLength = 64;
    ...
    addBtn.type = "button";
    addBtn.className = "soft-btn tag-add-btn";
    addBtn.textContent = "+";
    ...
    const addGroup = document.createElement("span");
    addGroup.className = "tag-add-group";
    addGroup.appendChild(input);
    addGroup.appendChild(addBtn);
    control.appendChild(addGroup);
