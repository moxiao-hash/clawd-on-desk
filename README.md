...
- **DeepSeek Harness** — Claude Code hook mechanism: dsh runs our command hook (`clawd-dsh-hook.js`) through its built-in `dsh-hooks-claude-code` bridge for state (`idle/thinking/working/attention/juggling`), plus a native plugin for `error` reporting, permission bubbles (Allow / Always / Deny) and elicitation answers. Register with `npm run install:dsh-bridge` (or it syncs automatically when the dsh home exists). See [docs/guides/deepseek-harness-integration.md](docs/guides/deepseek-harness-integration.md).
...
### Permission Bubble
- **In-app permission review** — when Claude Code, Codex CLI, CodeBuddy, opencode, Pi, or DeepSeek Harness request supported tool permissions, Clawd pops a floating bubble card instead of waiting in the terminal
